...
[^codebook]: By "codebook" we mean a human and machine readable file that lists each column in a dataset and explains their meaning and the possible values they can take. This is an approach that is widely recommended but certainly not universally adopted (@horstmann2020). It is worth pointing out that there are other ways of describing a dataset that would be able to avoid redundancies in the descriptions we mention here. We will return to this issue in the *Metadata* section below.
...
| subject_id | questions_id | question_repetition | question_text           | response | response_time |
| ---------- | ------------ | ------------------- | ----------------------- | -------- | ------------- |
...
Considering questionnaire data, if one organizes the data table so that each row represents a person (as in Table 1), one implicitly makes the statement that the observational unit is at the person level (and thus that each column in that person's row describes a specific attribute of that person). Alternatively--and this is the view that we hold--one may decide that the observational unit is the "response", which is formed by the interaction between a person and a question (as in Table 2). In this case, each row in the data table would be indexed by an instance of a person x question encounter. This implies that the data from a given person will be spread over multiple rows in the data table since a person will typically answer many questions. This way of structuring data is less common for questionnaire data but is more typical of data in cognitive computerized testing, where each row represents a "trial" (see https://behaverse.org/data-model/spec/trials/).
...
Comparing Tables 1 and 2 one may appreciate that the exact same information may be organized into tabular form in various ways. As stated earlier, while the tidy data format is widely recommended as the *default* format for a tabular dataset [@wickham2014], depending on the specific needs, data analysts will have to reshape that table, for example from long to wide (or vice-versa). Modern data analysis software provides well-designed tools for reshaping data: for an example of sample code in R using the tidyverse package to convert the tidy format into a wide one, see code chunk 1.
...
](figures/fig_1.png){#fig-1 width=100% fig-alt="The same data is shown in two separate tables: the table on the top uses the wide format and the table on the bottom uses the tidy format. The image also contains text annotations and arrows to explain the following: the tidy table contains 24 values distributed over four columns (i.e., subject_id, q_id, text, and resp). Only one of those values is an NA value. For illustration purposes, that NA value is colored in grey. When that same data is formatted in a wide data table (i.e., the table at the bottom), the number of columns increases and the number of rows decreases: the total number of values in the wide data format is 45 which is almost twice as many as when that data was organized using the tidy format. Importantly, NA values are injected in the wide data format for combinations of variables and cases that did not occur (e.g., subject 002 was not asked question 1). For illustration purposes, these additional NA values are colored in orange."}
...
![Selecting the data referring to a question of interest is harder under the wide than under the tidy data format.](figures/fig_2.png){#fig-2 width=50% fig-alt="Two tables are shown side by side, representing the same data organized using either the wide format (left table) or the tidy format (right table). In the wide table, several columns are highlighted in blue and there is a corresponding text annotation saying \"It is difficult to select all the data from a particular question because that data is spread across multiple columns and we have to \"manually\" keep track of (and type) the column names.\". In the tidy table, a set of rows and columns are highlighted in blue and the corresponding text annotation says \"Here selecting the data from a particular question is a breeze: one simply needs to remember the name of the question--all the corresponding attributes are neatly grouped within a row.\""}
...
![Changing or augmenting response data with additional attributes (e.g., response times) requires adding many columns in the wide data frame (one per response), which means more effort/code and implies an increased risk of mistakes. In contrast, under the tidy data format, only a single additional column is needed per new attribute.](figures/fig_3.png){#fig-3 width=50% fig-alt="Two tables are shown side by side, representing the same data organized using either the wide format (left table) or the tidy format (right table). In the wide table, three columns are highlighted in blue and each value displays the + symbol. There's also a label next to each column saying \"New!\" and there is a text annotation saying \"To recode your variables (e.g., changing units from milliseconds to seconds) requires changing one by one each of the relevant columns\". In the tidy table (on the right), only one column is highlighted in blue, with the label \"New!\". The text annotation to explain this highlight says \"The same operation is straightforward in this case as only one column is affected.\""}
...
![Metadata are more concise and easier to manage under the tidy format than under the wide data format.](figures/fig_4.png){#fig-4 width=50% fig-alt="Two schematic diagrams are displayed side by side. The one on the left represent the case of the wide data format and the one on the right represents the case of the tidy data format. In both diagrams, the data is represented by a blue rectangle illustrating the shape of the corresponding data table, and the corresponding metadata is shown as one or multiple orange rectangles, again with a shape representing the shape of the corresponding tables. In the case of the wide format, there is only one metadata rectangle which is as long as the data is wide. The text annotation says \"Describing this data table is tedious and redundant because many columns have a redundant meaning.\" In the tidy format diagram on the right, there are three small, orange rectangles with the labels \"attributes\", \"questions\" and \"options\". The text annotation in that diagram says \"Here the description of the data is much easier and more effective because each response attribute, question and option are each described only once.\"."}


Metadata should describe data in a machine readable way; this makes it possible for software to take advantage of that metadata (e.g., for finding particular datasets, for data validation). A common way to structure such metadata is to list and explain, in a table for example, all the columns that are in a data table as well as all the possible values those columns can take (this is sometimes called a codebook; see also footnote 4). Describing a dataset in this way is convenient because one can readily query the meaning of a particular column in a dataset simply by looking up that particular entry in its codebook.  
 

Describing data in such a codebook is much easier under the tidy data format than it is under the typical wide data format (see @fig-4). Assuming a questionnaire with 100 questions, 3 attributes per question and 2 repetitions, the data table would have 601 columns (100x3x2 + 1 subject_index) under wide data format, each of which requiring its own entry in a codebook (i.e., 601 rows in a codebook table) generating large files with considerable redundancies and thus a higher risk for errors. Alternatively, under the tidy data format, the same data would have only 6 columns (subject_index, question_id, repetition_index and the three attributes). The 100 questions could have their own codebook (i.e., 100 entries, one per question) as well as the different types of response options offered for each question (e.g., with only 1 entry if all the questions used the same Likert scale).  Thus, in total, under the tidy data model, in our example, the codebook would have only 107, non-redundant entries, which is much smaller and more convenient to use than the 601 codebook entries under the typical wide data format.

Another important concern when using questionnaires is how to deal with multiple languages. In international projects the same questionnaires are typically distributed in multiple languages. Moreover, the language used by respondents in a study may not be the language required by the journal where the study will be published nor the language used by the scientific community that may reuse that dataset. Managing multiple languages under the wide data format can be quite cumbersome, leading to multiplication of columns and data documentation. Under the tidy data format, the English text (or an English key) could be used in the main data file together with a column indicating in which language a given question was asked. The translations of the question may then be compactly stored in dedicated Question and Option tables that can be joined using keys (e.g., `question_id`).


One may of course argue that instead of listing and explaining all individual columns in the metadata one could instead provide a sort of tutorial on how to decipher the column names. For example, one could state that "q<id>" refers to the id of the question (e.g., `q23_response_time_r2` is about question 23), that "_r<integer>" refers to the number of the repetition of that question and that the text between these two, describes the attribute encoded in the column (in our example, `response_time`). With this knowledge, the data analyst could then search for corresponding information (e.g., search what question `23` corresponds to). This strategy has the advantage of avoiding the redundancies we ascribed to the wide data format codebooks. However, this process requires substantially more effort on the data analysts side and is more error prone. Furthermore, this type of data documentation is likely not machine readable, which would considerably limit its usefulness. Most importantly, it is interesting to note that what this approach is in fact doing is instructing the data analyst to tidy the data in their own working memory, "reversing" the algorithm that was used to create the column names and then to look up specific information in tidy documentation tables (e.g., "look up entry `23` in the Question table"). In this context, adopting the tidy questionnaire data format with its corresponding tidy codebook appears to be a more principled, robust and user-friendly way of documenting tabular data.     
...
# The behaverse data model for questionnaires


In previous sections we presented the general principles of tidy data; here we want to provide a concrete illustration of how those principles apply to questionnaire data as well as provide additional specifications (not covered in the principles) regarding in particular what columns to include in the table, how to name things and what units to choose. This specification is part of a larger initiative on data models for behavioral data ([https://behaverse.org/data-model/](https://behaverse.org/data-model/)); we will focus here only on key elements and refer the interested reader to the online documentation. 



![The behaverse data model involves a set of linked tables. The main table is the "Response" table as it contains most of the information required to make sense of how an agent interacted with an item (which is composed of a stimulus and response options). More detailed information about any entity involved in a study is readily accessible via the Response table as it contains for each response keys that connect it to other tables (e.g., a Stimulus table that contains detailed information about the stimuli presented to the agent, including their timing). This figure illustrates the overall architecture of the behaverse data model; a more detailed diagram is available on the [behaverse website](https://behaverse.org/data-model/assets/schemas/trials_uml.png).](figures/fig_5.png){#fig-5 width=50% fig-alt="Entity-relationship diagram. There are 6 entities represented as boxes with the labels \"Agent\", \"Response\", \"Stimulus\", \"Option\", \"Evaluation\" and \"Scoping\".  The box \"Response\" is shown at the center with a thicker outline: all other boxes are visually connected only to the \"Response\" box via lines whose endings indicate the type of mapping between the connected tables (i.e., one-to-many, many-to-many)."}





Two points are worth noting at this stage. First, studies involve multiple types of data (e.g., metadata, administrative data, data about the instruments used, code books). Here we focus only on the table describing the interaction between an agent and a questionnaire item. We call this table the "Response" table (see @fig-5); we may also refer to it as the "main table" and all other tables as "secondary tables". Secondly, it is impossible to name things in a way that satisfies everyone. To encompass a large number of use cases we tend to prefer more abstract terms to designate a particular entity, which however may feel odd for people working on a specific use case. For example, we use the term "agent" rather than more specific terms like "respondent", "participant" or "subject" to cover a wider range of use cases (e.g., artificial agents or non-human animals performing computerized tests). 
...
It is worth noting that several of these concepts are confounded in many current questionnaire datasets, which can be quite problematic. For example, when responses are encoded using only a single numeric value (e.g., 2, 3 4) it is unclear if these values represent the index of the selected options, a numeric value associated to the label of the option or a score associated to a particular way of responding to that particular question. If there is documentation stating those numbers reflect a score, it is typically impossible to verify if the encoding is accurate in the absence of the actual responses in the data. The data model presented here allows to make a clear distinction between these concepts and is thus less error prone. 
...
Contributed to Conceptualization: PCL, MA

Contributed to Funding acquisition: PCL

Contributed to Methodology: PCL, MA

Project administration: PCL, MA

Contributed to Software: PCL, MA

Contributed to Validation: PCL, MA

Contributed to Writing – original draft: PCL

Contributed to Writing – review & editing: PCL, MA
...
The supplementary material contains a very small dataset that is freely and openly available. This dataset is part of a larger study which was approved by the local Ethics Review Panel.
