...
![The PROJECT data model involves a set of linked tables. The main table is the “Response” table as it contains most of the information required to make sense of how an agent interacted with an item (which is composed of a stimulus and response options). More detailed information about any entity involved in a study is readily accessible via the Response table as it contains for each response keys that connect it to other tables (e.g., a Stimulus table that contains detailed information about the stimuli presented to the agent, including their timing). This figure illustrates the overall architecture of the PROJECT data model; a more detailed diagram is available on the [PROJECT website](https://PROJECT.org/data-model/assets/schemas/trials_uml.png).](figures/fig_5.png){#fig-5 width=50%}
