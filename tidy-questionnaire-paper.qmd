...
The goal of this work is to contribute to a more consistent and effective use of questionnaires by offering recommendations for better structuring their data. This work is part of a larger initiative aiming to improve data standards in behavioral sciences and raise research quality (for more details, see the PROJECT_NAME website https://PROJECT_WEBSITE/standards/). 
...
# define example dataset in the long data format
df <- tribble(
    ~subject_id, ~question_id, ~question_text, ~answer, 
    "s001",   1, "Do you like cinnamon?", "yes", 
    "s001",   2, "How old are you?", "23", 
    "s002",   1, "Do you like cinnamon?", "no", 
    "s002",   2, "How old are you?", "37"
)
...
**Code Chunk 1.** Reshaping data from wide to long is a breeze with the `pivot_wider()` function from the tidyverse package. Note that there is also a `pivot_longer()` function for the reverse reshaping operation.
...
| ---------- | ------------ | ------------------- | ----------------------- | -------- | ------------- |
| s001       | 1            | 1                   | "Do you like cinnamon?" | "yes"    | 1.32          |
| s001       | 1            | 2                   | "Do you like cinnamon?" | "yes"    | 0.98          |
| s001       | 1            | 3                   | "Do you like cinnamon?" | "no"     | 3.78          | 
...
# join data tables
df_wj <- left_join(df_wide, demographics, by = "subject_id")
```
...
](figures/tidy_questionnaires_fig_1.png){#fig-1}
...
![Selecting the data referring to a question of interest is harder under the wide than under the long data format.](figures/tidy_questionnaires_fig_2.png){#fig-2}
...
# get all data corresponding to question 1
# -- under wide format
dd |> select(matches("^q1_"))
dd |> select(matches("r1$"))

# search for names that are either from question 1 or end with r2

# -- under the long format
df |> filter(question_id == "q1")
...
![Metadata are more concise and easier to manage under the long, tidy format than under the wide data format.](figures/tidy_questionnaires_fig_4.png){#fig-4}
...
# ---- scoring code ---- 
# this code could be separate from the data analysis script and reused on other datasets.

# define relevant questions for this scoring
question_selection <- c("q10", "q11")

# define scoring function (how to compute a score based on the responses to relevant questions)
my_scoring_function <- function(x){
  0.8*x$q10 + 1.2*x$q11
}


# ---- analysis: computing the scores ----
# computing scores typically follows this blueprint;
scores <- data %>% 
  
  # select only questions used for the scoring
  filter(question_id %in% question_selection) %>% 
  ...
  mutate(score = my_scoring_function(.)) %>% 
...
[PROJECT_NAME](https://PROJECT_WEBSITE/) is a collection of integrated projects and software systems designed to accelerate, integrate and scale-up cognitive science research to yield scientific discoveries and innovation while promoting best practices and open science.

The PROJECT_NAME data mode (PROJECT_ACRONYM)l--a central component of PROJECT_NAME--is born out of two key realizations. Firstly, while many datasets are now publicly available (e.g., on https://osf.io/ or https://zenodo.org/), it is very hard to find data that are readily usable. In fact, many datasets may not be usable at all, even after putting in considerable effort. Indeed, some data files are incomplete, lacking documentation, weirdly formatted and recorded using proprietary data formats. Secondly, as we were running a large scale study using many different cognitive tasks and questionnaires, we realized that if we were to analyze each task as a one-shot case, it would take us a lot of time and we would end up writing code that would be hard to reuse and to maintain. Furthermore, it would be difficult to enforce common principles across tasks and datasets (e.g., how to treat outliers, whether or not to log transform response times, whether or not to filter out incorrect responses) and hence it would be hard to ascertain if specific results are due to differences in data analytic approaches or to actual differences in the data. In short, data that follow specific conventions and standards make it possible a) to reuse other people’s data with minimal efforts and b) to develop standardized and reusable code.  
...
The PROJECT_NAME data model does not focus on such metadata; instead it focuses on how to describe and encode the main concepts of behavioral data (such as stimulus and responses) in the data set itself, how to consistently name the features of observations (i.e., known as a controlled vocabulary), what units of measurements to use and how to organize datasets into files and folders. 

A detailed description of the current version of PROJECT_ACRONYM can be found elsewhere (https://PROJECT_WEBSITE/data-model/). Here we describe only a handful of PROJECT_ACRONYM key design choices as they equally apply to questionnaire data.
...
# Contributions
Both authors contriubted to the design and writing of the paper.
